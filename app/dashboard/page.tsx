...
import { getHoldings, getNewsForTicker, SIMULATED_FAILURE_TICKER } from "@/lib/data";
...
export default function DashboardPage() {
  ...
  const news = useMemo(
    () => holdings.flatMap((h) => getNewsForTicker(h.ticker).map((n) => ({ ...n, companyId: h.id }))),
    [holdings]
  );
  ...
  const failedHolding = holdings.find((h) => h.ticker.toUpperCase() === SIMULATED_FAILURE_TICKER);
        ...
        <>
          ...
          {failedHolding && (
            <div className="mt-4 flex items-start gap-2 rounded-lg border border-[#EEDFC2] bg-[#FCF6E8] px-4 py-3 text-xs text-[#7A5E14]">
              ...
              Für {failedHolding.name} konnten aktuell keine News abgerufen werden (Quelle nicht erreichbar). Die
              übrigen {holdings.length - 1} Unternehmen wurden erfolgreich verarbeitet.
            </div>
          )}
