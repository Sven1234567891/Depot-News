...
import { getActiveHoldingById } from "@/lib/portfolio";
import { getNewsProvider } from "@/lib/newsProvider";
import { Holding, NewsItem } from "@/lib/types";
import { RelevanceBadge, SentimentBadge, ImpactScore } from "@/components/badges";

export default function CompanyPage({ params }: { params: { id: string } }) {
  const [company, setCompany] = useState<Holding | null | undefined>(undefined);
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const h = await getActiveHoldingById(params.id);
      if (cancelled) return;
      setCompany(h ?? null);
      if (h) {
        try {
          const items = await getNewsProvider().searchNewsByTicker(h.ticker);
          if (!cancelled) setCompanyNews(items.sort((a, b) => b.impact - a.impact));
        } catch {
  ...
  if (company === undefined) {
    return <main className="mx-auto max-w-2xl px-6 py-16 text-center text-sm text-[#5B6576]">Lädt …</main>;
  }

  if (company === null) {
    return (
      <main className="mx-auto max-w-2xl px-6 py-16 text-center">
        <p className="text-sm text-[#5B6576]">Unternehmen nicht gefunden.</p>
        <Link href="/dashboard" className="mt-4 inline-block text-xs underline underline-offset-2">
